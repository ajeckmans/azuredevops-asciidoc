...
## 2024-05-24 - [Fix Path Traversal in include macros]
**Vulnerability:** The `resolvePath` function for AsciiDoc include macros blindly trusted paths starting with `/`, returning them immediately without running them through the directory traversal (`..`) check logic. This allowed an attacker to bypass checks using paths like `/../../etc/passwd`.
**Learning:** Absolute paths still need to be normalized and checked for directory traversal (`..`) sequences, especially when resolving user-controlled include paths. Starting with `/` does not guarantee a path is safe.
**Prevention:** Apply normalization and traversal detection logic uniformly to both absolute and relative paths, adjusting the base path appropriately but ensuring `..` boundaries are strictly enforced.
