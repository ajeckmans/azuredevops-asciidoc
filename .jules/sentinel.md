...
## 2024-07-19 - Fix Absolute Path Traversal
**Vulnerability:** The `resolvePath` function for AsciiDoc includes blindly trusted absolute paths (paths starting with `/`) and returned them directly without checking for directory traversal elements like `..`. This could allow accessing files outside the intended base directory using paths like `/../../secret.txt`.
**Learning:** Even when user input is treated as an absolute path, it must be normalized and validated to prevent directory traversal. Trusting the leading slash is not sufficient.
**Prevention:** Normalize all paths, including absolute ones, by merging them with the base directory or by applying the same dot-dot (`..`) stack resolution mechanism used for relative paths to ensure they stay within bounds.
