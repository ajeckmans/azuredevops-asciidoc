...
## 2023-10-27 - [Fix Path Traversal in AsciiDoc Path Resolver]
**Vulnerability:** The `resolvePath` function for AsciiDoc link and include resolution blindly returned the path without processing `..` segments if the target path started with `/`. This enabled absolute path traversal attacks (e.g., `/../../../etc/passwd`).
**Learning:** Checking for an absolute path does not make it safe from traversal. Even absolute paths must be normalized and checked for out-of-bound `..` segments.
**Prevention:** Split all paths (relative or absolute) and resolve them via a stack to safely eliminate `..` segments and block traversal attempts above the intended root.
