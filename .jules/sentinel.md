...
## 2024-05-18 - Path Traversal bypass via absolute paths in resolvePath
**Vulnerability:** In `resolvePath` function for AsciiDocRenderer include directives, paths that began with `/` were immediately returned without any validation, completely bypassing the directory traversal protection (`..` check) that was in place for relative paths. This allowed malicious includes to fetch arbitrary server paths or traverse upwards without constraint.
**Learning:** Returning early on an absolute path bypassing subsequent validation logic is a common anti-pattern. Validation checks (like `..` segment tracking) must be applied to ALL paths uniformly, regardless of whether they are absolute or relative.
**Prevention:** Normalize absolute and relative paths into a common structure *before* applying path traversal validation (e.g., block list or stack-based `..` collapsing).
