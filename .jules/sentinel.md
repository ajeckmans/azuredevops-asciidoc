...
**Vulnerability:** The `resolvePath` function for processing `include::` directives naively trusted any path starting with `/` (absolute paths) by returning it directly without checking for directory traversal sequences like `..`. This allowed an attacker to read arbitrary files from the filesystem via paths like `/../../etc/passwd`.
**Learning:** Checking if a path is absolute does not guarantee that the path has been normalized or is safe from traversal attacks. Traversal segments (`..`) can exist within absolute paths, allowing the path to escape its intended root boundaries.
**Prevention:** Always normalize and evaluate directory traversal checks (`..` handling via a stack or robust built-in URL resolver) for all paths, regardless of whether they are absolute or relative, before allowing file system access.
