...
**Vulnerability:** The custom `resolvePath` function immediately returned any target path that started with `/` (e.g. `/../../etc/passwd`), completely bypassing the stack-based `..` normalization and out-of-bounds checks for absolute paths.
**Learning:** Checking for an absolute path and returning early incorrectly assumes the path is already normalized and safe. Absolute paths can still contain traversal segments that break out of intended boundaries.
**Prevention:** Always subject both absolute and relative paths to full normalization and directory traversal limit checks. When processing an absolute path, parse it through the normal logic but treat the starting directory as empty/root.
