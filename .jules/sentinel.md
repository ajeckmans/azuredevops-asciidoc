...
## 2024-05-24 - [Fix absolute path traversal]
**Vulnerability:** Absolute paths in AsciiDoc rendering were trusted implicitly if they started with `/`. This allowed for absolute path traversals like `include::/../etc/passwd[]` that bypassed the traversal checks on relative paths.
**Learning:** Checking for directory traversal by breaking a path into a stack (pushing on valid paths and popping on `..`) is effective, but it must be applied to ALL paths uniformly, regardless of whether they are absolute or relative.
**Prevention:** Rather than an early return for paths starting with `/`, join absolute and relative paths accordingly first, and then process the unified string through the traversal checker loop to normalize and validate it securely.
