...
## 2024-05-20 - Fix Path Traversal in Absolute Paths
**Vulnerability:** Path traversal vulnerability in `resolvePath` function in `src/components/AsciiDocRenderer.tsx`. Paths starting with `/` were returned as-is without being sanitized for `..` segments, allowing malicious absolute paths like `/../../secret.txt` to traverse out of bounds.
**Learning:** Naively returning absolute paths allows out-of-bounds `..` segments to bypass path traversal checks. We must validate all components of absolute paths as well.
**Prevention:** All paths, including absolute paths, must be normalized and checked for out-of-bounds traversal (`..`) by evaluating the directory segments stack.
