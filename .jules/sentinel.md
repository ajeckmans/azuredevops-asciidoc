...
## 2024-05-18 - Path Traversal in Absolute Include Paths
**Vulnerability:** Path traversal in `resolvePath` where absolute paths starting with `/` bypassed normalization, allowing paths like `/../../secret.txt` to read outside allowed directories.
**Learning:** Returning early on `/` bypasses directory traversal prevention checks built into the path builder algorithm. All paths, including absolute ones, must go through traversal prevention (like stack popping `..`).
**Prevention:** Normalize and check all paths, handling both absolute and relative paths uniformly with `..` checks before resolving file content.
