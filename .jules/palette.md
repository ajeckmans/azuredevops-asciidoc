...
## 2026-07-20 - Add explicit tooltips for icon-only buttons
**Learning:** Azure DevOps UI icon-only buttons require explicit `Tooltip` wrapping (using `azure-devops-ui/TooltipEx`) to provide visual context for sighted users, as `aria-label` alone is not surfaced visually.
**Action:** Always wrap icon-only buttons with the `<Tooltip>` component.
