...
describe("AsciiDocRenderer", () => {
    ...
    it("prevents path traversal above root in include macros with absolute paths", async () => {
        const fetchFileContent = jest.fn().mockResolvedValue("secret content");
        const warnSpy = jest.spyOn(console, 'warn').mockImplementation(() => {});

        render(<AsciiDocRenderer content="include::/../../etc/passwd[]" filePath="/docs/main.adoc" fetchFileContent={fetchFileContent} />);

        await waitFor(() => {
            expect(warnSpy).toHaveBeenCalledWith("Path traversal blocked: /../../etc/passwd");
            expect(fetchFileContent).not.toHaveBeenCalled();
        });
